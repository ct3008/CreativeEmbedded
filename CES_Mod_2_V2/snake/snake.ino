...
void loop() {
  ...
  if (xVal < MIN_THRESHOLD && yVal > 0 && yVal < MAX_ANALOG_VALUE) {
  ...
  } else if (yVal < 150 && xVal >= 0 && xVal <= MAX_ANALOG_VALUE) {
    direction = 0;
    //UP
    ...
    ledcWrite(CHAN_LED_3, 0);
    ledcWrite(CHAN_LED_4, 0);
  } else if (yVal >= 3945 && xVal >= 0 && xVal <= 4095) {
    direction = 2;
    //DOWN
    ...
    ledcWrite(CHAN_LED_2, 0);
    ledcWrite(CHAN_LED_4, 0);
  } else if (xVal > MAX_THRESHOLD && yVal > 150 && yVal < MAX_ANALOG_VALUE) {
    direction = 3;
    //LEFT
